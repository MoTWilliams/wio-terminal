...
namespace {
        ...
        // Received requests
        // constexpr char const* PATH_RECORD = "/record/start";
        // constexpr char const* PATH_STATUS = "/status";

        // constexpr char const* ACTION_RECORD = "Record 3 seconds";
        // constexpr char const* ACTION_STATUS = "Report system status";
        
        // // Sent requests
        // constexpr char const* PATH_RECORDING_STARTED = "/recording/started";
        // constexpr char const* PATH_RECORDING_STOPPED = "/recording/stopped";
        
        // constexpr char const* ACTION_RECORDING_STARTED = "Recording started";
        // constexpr char const* ACTION_RECORDING_STOPPED = "Recording stopped";
        
        // Error or whatever
        // constexpr char const* ACTION_NOT_HANDLED = "Action not handled";
}

HTTPConnection::HTTPConnection(WiFiClient &client, LineBuffer &buffer)
        : client(client), lineBuffer(buffer) {}
...
bool HTTPConnection::readChar(char &c) {
        if (!System::connectionMonitor.checkConnection(client, lastProgress))
                { reset(); return false; }
        ...
        {
                reset();
                System::connectionMonitor.handleConnectionLost();
                return false;
        }
